...
5. Attach the PR URL to the issue (Linear attachment preferred; workpad only if attachment isn't possible). Add `symphony` label on the GitHub PR. For user-facing changes, embed the same Playwright screenshots in the PR description (markdown image syntax pointing at uploaded GitHub assets). Do not commit proof screenshots to the repository.
...
- **Proof-of-testing screenshots must be Playwright-captured** for user-facing changes, and must appear in BOTH the Linear workpad and the PR description via uploaded assets or attachments. Do not check screenshot files into the repository. Hand-cropped screenshots, `console.log` snippets, or text-only descriptions do not satisfy this requirement.
